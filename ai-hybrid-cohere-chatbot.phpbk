...
class AI_Support_Chatbot {
    ...
    public function __construct() {
        ...
        // new endpoint to let front‑end poll for human agent replies
        ...
        // new endpoint to check agent availability
        ...
        // Agent heartbeat (admin users poll this to stay "online")
    ...
    public function stream_chat() {
        ...
        // If the chat session has been escalated to a human agent, no longer respond with AI text.
    ...
    public function request_escalation() {
        ...
        // If no agents are currently available, return contact info instead of queuing for an agent.
        $status = '';
        $pending = 0;
        $agent_available = $this->is_agent_available($status, $pending);
        if (!$agent_available) {
    ...
    public function chatbot_ui() {
        ...
        <style>
            ...
            /* agent bubble should stand out slightly from AI */
        ...
        <script>
            (function(){
                ...
                // restore session id if previously created
                let sessionId = null;
                try{
                ...
                if(sessionId){
                    // resume polling later once helper is defined (see below)
                }
                ...
                function appendMessageToDOM(message, incrementUnread = true){
                    // message: { role: 'user'|'assistant'|'agent', content, ts, isTyping? }
                ...
                toggleBtn.addEventListener('click', function(){
                    ...
                    if(open){
                        ...
                        if(userData && userData.name && userData.email){
                            ...
                            // If we have a session, update the agent status display
                            if(sessionId){
                ...
                startBtn.addEventListener('click', function(){
                    ...
                    fetch(ajaxUrl, {
                        ...
                        if (data.success && data.data && data.data.session_id) {
                            ...
                            // once we have a session, check agent availability
                            checkAgentAvailability();
                            // Check availability initially and then every 6 seconds
                            setInterval(checkAgentAvailability, 6000);
                            // once we have a session, start polling for agent replies
                            pollAgentMessages();
                            // poll every 4 seconds while the widget is open
                ...
                let agentAvailable = false; // Track if agent is available

                // Function to check agent availability and update status display
                ...
                // Function to update the agent status display
                function updateStatusDisplay(){
                    const statusDot = document.getElementById('ai-status-dot');
                    const statusText = document.getElementById('ai-status-text');
                    const requestBtn = document.getElementById('ai-request-agent-btn');
                    
                    // Always show the request button, but only enable it when an agent is available.
                ...
                let lastAgentId = 0;   // highest agent message id we have displayed

                // helper that polls server for any new agent replies
                ...
                // ensure polling starts if we already restored a session
                if(sessionId){
                    checkAgentAvailability();
                    setInterval(checkAgentAvailability, 6000);
                    pollAgentMessages();
                ...
                // Request Agent button handler (replaces old escalate button)
    ...
    // Polling API used by frontend widget to pull any agent messages
    private function is_agent_available(&$status = null, &$pending_count = null) {
        global $wpdb;

        $session_table = $wpdb->prefix . 'ai_chat_sessions';
        $pending_count = intval($wpdb->get_var(
            "SELECT COUNT(*) FROM $session_table WHERE status IN ('waiting', 'human_handling')"
        ));

        $agent_status = get_option('ai_agents_online_status', 'offline');
        $status = $agent_status;

        $agent_available = ($agent_status === 'online');

        // If any agent has recently pinged the server, consider them online.
        ...
        // If there are pending chats, assume an agent is actively handling them.
        if ($pending_count > 0) {
            $agent_available = true;
        }
